...
- визуальный жанр экранов: часть страниц выглядит как operational workspace, часть как showcase/dashboard из другого продукта

Именно поэтому этот раздел фиксирует target state заранее, даже если кодовая база ещё не полностью к нему приведена.

## Почему этот refactor вообще нужен

Цель refactor не в том, чтобы механически проставить одинаковые `rounded-*` по всему проекту.

Главная причина:

- соседние app-страницы иногда решены в разных визуальных жанрах
- из-за этого даже качественный локальный экран может выглядеть “как из другого проекта”
- если исправлять только токены, но не учитывать жанр экрана, можно легко сломать удачную страницу

Практический вывод:

- сначала нужно понять, что за экран перед нами: operational workspace, profile-like page, dashboard/feed, auth/legal flow
- потом сохранять удачный внешний контракт этого типа экрана
- и только после этого выравнивать геометрию, surfaces, controls и shared affordance
