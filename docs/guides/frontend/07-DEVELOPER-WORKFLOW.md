...
1. определить жанр экрана: operational workspace, profile-like page, dashboard/feed или auth/legal flow
2. понять, это новый паттерн или существующий
3. найти ближайшие shared primitives
4. определить уровень поверхности и геометрии
5. собрать layout из существующих слоёв
6. вынести тяжёлые feature-блоки из page-level файла, если экран уже растёт за счёт list row / detail modal / compose workspace
7. только потом добавлять локальные override
...
- определить, это рабочий модуль, profile-like page или обзорный dashboard/feed
...
- если экран уже визуально удачный, первый шаг рефакторинга должен сохранять внешний контракт и чистить структуру под капотом, а не навязывать новый layout только ради декомпозиции
- если у toolbar/filter block пропсы начинают расползаться, можно группировать их локально, но это не должно само по себе заставлять перепридумывать успешный внешний вид блока
- если экран ощущается как “из другого проекта”, причина чаще всего не в одном radius или цвете, а в несовпадении screen genre; это нужно решать на уровне hierarchy, density и panel semantics, а не точечными utility-правками
