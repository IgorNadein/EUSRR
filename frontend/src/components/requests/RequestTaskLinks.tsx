...
import type { Request, TaskBoard, TaskCard, TaskPriority, User } from "@/types/api";

type RequestTaskLinksProps = {
  request: Request;
...
function requestTitle(request: Request) {
  return request.display_title || request.title || `Заявление #${request.id}`;
}

export function RequestTaskLinks({
  request,
  variant = "section",
  open = false,
  onClose,
  onLinked,
}: RequestTaskLinksProps) {
  ...
  const loadLinkedTasks = useCallback(async () => {
    setLoading(true);
    try {
      const tasks = await apiClient.getRequestLinkedTasks(request.id);
  ...
  const prepareLinkModal = useCallback(async () => {
    ...
    setTitle(`Задача по заявлению: ${baseTitle}`);
    setDescription(request.comment || request.description || "");
    try {
  ...
  const saveLink = async () => {
    ...
    try {
      ...
      await apiClient.linkTaskRequest(taskToLink.id, request.id);
      await loadLinkedTasks();
      toast.success("Заявление связано с задачей");
      onLinked?.();
      if (dialogMode) {
        onClose?.();
      } else {
      ...
      setError(saveError instanceof Error ? saveError.message : "Не удалось связать заявление с задачей");
    } finally {
      setSaving(false);
    }
  };
    ...
    >
      <div className="space-y-4">
        <div className="app-surface-muted rounded-xl border border-[var(--border-subtle)] p-3">
          <p className="app-card-caption">Заявление</p>
