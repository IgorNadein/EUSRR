...
export function EmployeeTaskLinks({
  employee,
  variant = "section",
  open = false,
  onClose,
  onLinked,
}: EmployeeTaskLinksProps) {
  ...
  const loadLinkedTasks = useCallback(async () => {
    setLoading(true);
    try {
      const tasks = await apiClient.getEmployeeLinkedTasks(employee.id);
  ...
  const prepareLinkModal = useCallback(async () => {
    setSaving(true);
    setError(null);
    setTitle(`Задача по сотруднику: ${employeeName}`);
    setDescription(employee.position?.name ? `Должность: ${employee.position.name}` : "");
    try {
  ...
  }, [employee.id, employee.position?.name, employeeName, linkedTaskIds]);
  ...
  const saveLink = async () => {
    ...
    try {
      ...
      await apiClient.linkTaskEmployee(taskToLink.id, employee.id);
      await loadLinkedTasks();
      toast.success("Сотрудник связан с задачей");
      onLinked?.();
      if (dialogMode) {
        onClose?.();
      } else {
      ...
      setError(saveError instanceof Error ? saveError.message : "Не удалось связать сотрудника с задачей");
    } finally {
      setSaving(false);
    }
  };
    ...
    >
      <div className="space-y-4">
        <div className="app-surface-muted rounded-xl border border-[var(--border-subtle)] p-3">
          <p className="app-card-caption">Сотрудник</p>
          <p className="app-text-wrap mt-1 line-clamp-3 text-sm text-[var(--foreground)]">
            {employeeName}
          </p>
          <p className="app-text-muted mt-1 text-xs">
            {[employee.position?.name, employee.email].filter(Boolean).join(" · ")}
          </p>
        </div>
              ...
              >
                <span>
                  ...
                  <span className="app-text-muted mt-0.5 block text-xs">
                    Исполнитель, срочность, срок и колонка
                  </span>
                </span>
                <ChevronRight
                  size={16}
