...
export default function MessageComposer({
  ...
  return (
    <>
      {editingMessageId ? (
        <div className="mb-2 flex items-start justify-between gap-2 rounded-lg border border-amber-300/60 bg-amber-500/10 px-3 py-2 text-xs text-amber-700">
          ...
          <button
            ...
            className="rounded-full p-0.5 text-amber-700 transition hover:bg-amber-500/10"
      ...
      {replyTo ? (
        <div className="mb-2 flex items-start justify-between gap-2 rounded-lg border border-sky-400/30 bg-sky-500/10 px-3 py-2 text-xs text-sky-800">
          <div className="min-w-0">
          ...
          <button
            ...
            className="rounded-full p-0.5 text-sky-700 transition hover:bg-sky-500/10"
        ...
        <div className="mb-2 flex flex-wrap gap-2">
          {attachedFiles.map((file, index) => (
            <span
              ...
              className="inline-flex max-w-full items-center gap-1 rounded-full border border-sky-400/20 bg-[var(--surface-elevated)] px-3 py-1 text-xs text-sky-700"
            >
              <span className="truncate max-w-[180px]">{file.name}</span>
              <button
                type="button"
                onClick={() => onRemoveFile(index)}
                className="rounded-full p-0.5 transition hover:bg-sky-500/10"
      ...
      {canSendMessages ? (
        <div className="flex items-end gap-2">
          ...
          >
            <button
              ...
              className={`app-icon-button absolute left-3 z-10 inline-flex h-7 w-7 items-center justify-center rounded-full focus:outline-none focus:ring-2 focus:ring-sky-100 disabled:cursor-not-allowed disabled:opacity-50 ${
            ...
            <button
              ...
              className={`app-icon-button absolute right-3 z-10 inline-flex h-7 w-7 items-center justify-center rounded-full focus:outline-none focus:ring-2 focus:ring-sky-100 ${
                isExpanded ? "bottom-2.5" : "top-1/2 -translate-y-1/2"
              }`}
              title="Смайлы"
            ...
            <textarea
              ...
              className={`w-full resize-none bg-transparent py-2.5 pl-11 pr-12 text-sm leading-5 text-[var(--foreground)] outline-none placeholder:text-[var(--muted-foreground)] ${
        ...
        <div className="rounded-lg border border-amber-300/60 bg-amber-500/10 px-4 py-3 text-center text-sm text-amber-700">
