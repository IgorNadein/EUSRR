...
import type { Document, TaskBoard, TaskCard, TaskPriority, User } from "@/types/api";

type DocumentTaskLinksProps = {
  document: Document;
...
export function DocumentTaskLinks({
  document,
  variant = "section",
  open = false,
  onClose,
  ...
  const loadLinkedTasks = useCallback(async () => {
    setLoading(true);
    try {
      const tasks = await apiClient.getDocumentLinkedTasks(document.id);
  ...
  const prepareLinkModal = useCallback(async () => {
    setSaving(true);
    setError(null);
    setTitle(document.title ? `Задача по документу: ${document.title}` : "Задача по документу");
  ...
  }, [document.description, document.title, linkedTaskIds]);
  ...
  const saveLink = async () => {
    ...
    try {
      ...
      await apiClient.linkTaskDocument(taskToLink.id, document.id);
      await loadLinkedTasks();
      toast.success("Документ связан с задачей");
      onLinked?.();
      if (dialogMode) {
        onClose?.();
      } else {
      ...
      setError(saveError instanceof Error ? saveError.message : "Не удалось связать документ с задачей");
    } finally {
      setSaving(false);
    }
  };
    ...
    >
      <div className="space-y-4">
        <div className="app-surface-muted rounded-xl border border-[var(--border-subtle)] p-3">
          <p className="app-card-caption">Документ</p>
