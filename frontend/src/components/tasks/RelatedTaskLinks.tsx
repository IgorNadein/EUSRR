...
type RelatedTaskLinksProps = {
  ...
  loadLinkedTasks: () => Promise<TaskCard[]>;
  linkTask: (taskId: number) => Promise<unknown>;
...
export function RelatedTaskLinks({
  ...
  const refreshLinkedTasks = useCallback(async () => {
    setLoading(true);
    try {
      setLinkedTasks(await loadLinkedTasks());
    } catch (loadError) {
  ...
  const prepareLinkModal = useCallback(async () => {
    setSaving(true);
    setError(null);
    setTitle(defaultTaskTitle);
    setDescription(defaultTaskDescription);
    try {
  ...
  }, [defaultTaskDescription, defaultTaskTitle, linkedTaskIds]);
  ...
  const saveLink = async () => {
    ...
    try {
      ...
      setError(saveError instanceof Error ? saveError.message : "Не удалось связать объект с задачей");
    } finally {
      setSaving(false);
    }
  };
    ...
    >
      <div className="space-y-4">
        <div className="app-surface-muted rounded-xl border border-[var(--border-subtle)] p-3">
          <p className="app-card-caption">{entityLabel}</p>
          <p className="app-text-wrap mt-1 line-clamp-3 text-sm text-[var(--foreground)]">
            {entityTitle}
          </p>
          {entitySubtitle ? (
            <p className="app-text-muted mt-1 text-xs">{entitySubtitle}</p>
          ) : null}
        </div>
