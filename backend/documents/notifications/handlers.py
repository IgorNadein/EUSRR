...
def notify_specific_users(document, user_ids):
    ...
    for user_id in user_ids:
        try:
            ...
            logger.error(f"[handlers] Error creating notification for user {user_id}: {e}")
...
def notify_department_employees(document, department_ids):
    ...
    for employee in all_employees:
        try:
            ...
            logger.error(f"[handlers] Error creating notification for employee {employee.id}: {e}")
...
def notify_document_comment(document, comment):
    """
    Уведомляет автора документа о новом комментарии.
    
    Args:
        document: Объект Document
        comment: Объект DocumentComment
    """
    author = comment.author
    
    if not document.uploaded_by:
        logger.info(
            "[handlers] Document has no author, skipping"
        )
        return
    ...
    if doc_author.id == author.id:
        logger.info(
            "[handlers] Skipping self-comment notification"
        )
        return
    
    logger.info(
        f"[handlers] Notifying document author={doc_author.pk} "
        f"about new comment"
    )
    ...
    notify.send(
        ...
        verb=NotificationVerbs.DOCUMENT_COMMENT,
        action_object=comment,
        description=MessageTemplates.comment(author_name, document.title),
        action_url=ActionURLs.DOCUMENTS,
        data={
...
def notify_comment_reply(parent_comment, reply_comment):
    """
    Уведомляет автора комментария об ответе на него.
    
    Args:
        parent_comment: Объект DocumentComment (родительский)
        reply_comment: Объект DocumentComment (ответ)
    """
    author = reply_comment.author
    parent_author = parent_comment.author
    document = reply_comment.document
    
    # Не уведомляем самого себя
    if parent_author.id == author.id:
        logger.info(
            "[handlers] Skipping self-reply notification"
        )
        return
    
    logger.info(
        f"[handlers] Notifying parent author={parent_author.pk} about reply"
    )
    ...
    notify.send(
        ...
        verb=NotificationVerbs.DOCUMENT_COMMENT_REPLY,
        action_object=reply_comment,
        description=MessageTemplates.comment_reply(author_name, document.title),
        action_url=ActionURLs.DOCUMENTS,
        data={
            'title': MessageTemplates.comment_reply_title(),
            'document_id': document.id,
