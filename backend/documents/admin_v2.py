# documents/admin_v2.py
"""
Django admin для новых моделей DocumentV2 и DocumentAcknowledgementV2.

Основные улучшения:
- Использование AdminFileWidget и AdminFileFieldWrapper от django-filer
...
class AcknowledgementV2Inline(admin.TabularInline):
    """Inline для отображения ознакомлений с документом."""
    model = DocumentAcknowledgementV2
    extra = 0
    can_delete = False
...
@admin.register(DocumentV2)
class DocumentV2Admin(admin.ModelAdmin):
    """
    Admin для DocumentV2 с поддержкой django-filer.
    ...
    """
    inlines = [AcknowledgementV2Inline]
    ...
    def acknowledgement_status(self, obj):
        ...
        acked_count = DocumentAcknowledgementV2.objects.filter(document=obj).count()
    ...
    def send_document(self, request, queryset):
        ...
        # TODO: обновить tasks.py для работы с DocumentV2
        self.message_user(request, _("⚠️ Рассылка пока не реализована для DocumentV2"))
    ...
    def pending_list(self, obj):
        ...
        recipients = set(self.get_recipients_qs(obj))
        acked_ids = set(
            DocumentAcknowledgementV2.objects
            .filter(document=obj)
...
@admin.register(DocumentAcknowledgementV2)
class DocumentAcknowledgementV2Admin(admin.ModelAdmin):
