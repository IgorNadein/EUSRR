# documents/tests.py
"""
Тесты для моделей Document и DocumentAcknowledgement с django-filer
"""
...
class DocumentAcknowledgementModelTest(TestCase):
    """Тесты для модели DocumentAcknowledgement"""
...
class DocumentRulesTest(TestCase):
    ...
    def test_owner_can_view(self):
        ...
        self.assertTrue(
            rules.test_rule('documents.view_document', self.owner, self.document)
        )
    
    def test_other_user_cannot_view(self):
        """Другой пользователь не может просматривать чужой документ (если не в списке)"""
        self.assertFalse(
            rules.test_rule('documents.view_document', self.other_user, self.document)
        )
    ...
    def test_owner_can_edit(self):
        ...
        self.assertTrue(
            rules.test_rule('documents.change_document', self.owner, self.document)
        )
    
    def test_other_user_cannot_edit(self):
        """Другой пользователь не может редактировать чужой документ"""
        self.assertFalse(
            rules.test_rule('documents.change_document', self.other_user, self.document)
        )
    
    def test_owner_can_delete(self):
        """Владелец может удалять свой документ"""
        self.assertTrue(
            rules.test_rule('documents.delete_document', self.owner, self.document)
        )
    
    def test_other_user_cannot_delete(self):
        """Другой пользователь не может удалять чужой документ"""
        self.assertFalse(
            rules.test_rule('documents.delete_document', self.other_user, self.document)
        )
