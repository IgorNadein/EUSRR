...
class Document(models.Model):
    ...
    folder = models.ForeignKey(
        ...
        related_name="documents",
        help_text=_(
            "Папка для организации документов в иерархической структуре"
        ),
    )
    ...
    sent_to_all = models.BooleanField(
        ...
        default=True,
        help_text=_(
            "Если включено — уведомление получат все активные сотрудники"
        ),
    )
    ...
    departments = models.ManyToManyField(
        ...
        blank=True,
        help_text=_(
            "Документ будет доступен всем сотрудникам "
            "выбранных отделов (включая будущих)"
        ),
        related_name="documents",
    )
    ...
    def get_thumbnail(self, size="medium"):
        ...
        try:
            ...
            # Если thumbnail не удалось создать (не изображение),
            # возвращаем иконку по
            # типу
...
class DocumentAcknowledgement(models.Model):
    ...
    acknowledged_at = models.DateTimeField(
        _("Время ознакомления"), auto_now_add=True
    )
    ...
    def __str__(self):
        return (
            ...
            f"{self.acknowledged_at:%d.%m.%Y %H:%M}"
        )
...
class DocumentType(models.Model):
    ...
    metadata_schema = models.JSONField(
        ...
        blank=True,
        help_text=_(
            "JSON Schema для валидации метаданных документов этого типа"
        ),
    )
...
class DocumentTag(models.Model):
    ...
    documents = models.ManyToManyField(
        Document, related_name="tags", blank=True, verbose_name=_("Документы")
    )
...
class DocumentAuditLog(models.Model):
    ...
    action = models.CharField(
        _("Действие"), max_length=50, choices=Action.choices
    )
