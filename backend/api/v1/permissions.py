...
def user_is_dept_head(user, dept: Department) -> bool:
    return bool(
        ...
        and dept.head_id == getattr(user, "id", None)
    )
...
def user_has_dept_perm(user, dept: Department, perm_code: str) -> bool:
    ...
    if not isinstance(dept, Department) or getattr(dept, "id", None) is None:
        raise ValueError(
            "Argument `dept` must be Department instance with a valid id."
        )
    ...
    # Проверка через RoleAssignment (новая модель — не требует членства в
    # отделе)
    ...
    # Fallback: старая проверка через EmployeeDepartment.role (обратная
    # совместимость)
...
def has_dept_perm(
    user: AbstractBaseUser, department_id: int, code: str
) -> bool:
    """
        Возвращает True, если пользователь имеет право `code`
        в рамках отдела `department_id`.

    Логика:
      1) staff/superuser → всегда True
            2) пользователь является текущим руководителем отдела → True
                 для любых «управленческих» кодов
            3) пользователь имеет активное назначение роли
                 (RoleAssignment) с правом `code` → True
    ...
    # 3) Проверка через RoleAssignment (новая модель — не требует членства в
    # отделе)
    if RoleAssignment.objects.filter(
        employee_id=user.id,
    ...
    link = (
        ...
        .filter(
            employee_id=user.id, department_id=department_id, is_active=True
        )
        .first()
    )
...
class AdminOrDeptAllowed(BasePermission):
    """Комбинированный пермишен: админский доступ ИЛИ доступ
    по праву на конкретный отдел.
    ...
    Поведение:
        ...
        3. Идентификатор отдела извлекается из:
           ...
           На уровне has_object_permission отдел берётся из самого объекта:
             ...
             - obj.department.id.
          4. Если код права не задан (required_code/required_code_map
              пусты), то для SAFE-методов (GET/HEAD/OPTIONS) доступ
              разрешён, если allow_safe_without_code=True
              (по умолчанию).
           Для небезопасных методов в этом случае доступ запрещён.

    Атрибуты класса:
        required_code (str|None): Единый код права для всех экшенов.
            Если None, используется required_code_map.
        required_code_map (dict[str,str]): Коды прав по имени action
            (create/update/partial_update/destroy/...).
        allow_safe_without_code (bool): Разрешать ли SAFE-методы,
    ...
    def get_required_code(self, request, view) -> Optional[str]:
        ...
        Сначала берёт из required_code_map по имени action
        (DRF: view.action), затем fallback на required_code.

        Args:
            request: DRF Request (не используется в текущей логике).
            view: DRF View/ViewSet; важно наличие атрибута 'action'
                для ViewSet.

        Returns:
            Optional[str]: Код права или None, если право для
                экшена не требуется.
        """
        return (
    ...
    def _extract_dept_id_from_request(self, request, view) -> Optional[int]:
        """Пытается извлечь department_id из kwargs, body
        или query-параметров.
        ...
        Returns:
            Optional[int]: Значение dept_id, если найдено и приведено
                к int, иначе None.
        """
    ...
    def has_permission(self, request, view) -> bool:
        ...
        Если dept_id не найден, но это detail-запрос, откладываем
        проверку до object-level (вернём True).
        """
    ...
    def has_object_permission(self, request, view, obj) -> bool:
        ...
        Правила:
            - superuser/staff → True.
                        - Если код не требуется → SAFE-методы допускаются,
                            если allow_safe_without_code=True.
            - Иначе:
                - Пытаемся извлечь department_id из объекта;
                                - если не удалось — пробуем извлечь из запроса
                                    (kwargs/data/query);
...
class IsSelfOrStaff(BasePermission):
    ...
    DEFAULT_OWNER_OBJ_ATTRS: Tuple[str, ...] = (
        "employee",
        "author",
        "user",
        "owner",
    ...
    def has_object_permission(self, request: Request, view, obj) -> bool:
        ...
        Логика владельца:
            ...
            2) Совпадение связанных owner-объектов (employee/user/…)
                3) Фоллбек "self-object": obj.__class__ == user.__class__
                    и obj.pk == user.pk
        ...
        # 3) self-object: редактирование собственной учётки (Employee ↔
        # Employee)
        try:
            if obj.__class__ is user.__class__ and getattr(
                obj, "pk", None
...
class AdminOrActionOrModelPerms(DjangoModelPermissions):
    """staff/superuser ИЛИ явные пермы экшена ИЛИ стандартные
    DjangoModelPermissions.

    Логика:
      1) staff/superuser → True
        2) Если на вью заданы «явные» коды прав для текущего action
            (метод `_explicit_perm_codes`), достаточно ЛЮБОГО из них
            → True/False
    ...
    def _explicit_perm_codes(view) -> Optional[list[str]]:
        ...
        Поддерживаемые формы на ViewSet:
          ...
          - required_perms_by_action[action]: {method: str|Iterable[str]}
            где method сравнивается без учёта регистра,
            возможны '*', 'ANY', а HEAD трактуется как GET.
    ...
    def has_permission(self, request: Request, view) -> bool:
        ...
        # 1) Явные пермы, привязанные к action.
        # Если у тебя есть такой механизм.
