...
class ScheduleCalendarViewSet(viewsets.ModelViewSet):
    ...
    Endpoints:
        - GET /api/v1/schedule/calendars/ - список календарей
    ...
    - POST /api/v1/schedule/calendars/{id}/add_participant/ - добавить участника
        - DELETE /api/v1/schedule/calendars/{id}/remove_participant/
            - удалить участника
    ...
    pagination_class = (
        None  # Отключаем пагинацию - календарей немного, нужны все
    )
    ...
    def add_participant(self, request, pk=None):
        ...
        serializer = CalendarRelationSerializer(
            data={
    ...
    @action(
        detail=True,
        methods=["delete"],
        url_path="remove-participant/(?P<user_id>[0-9]+)",
    )
    def remove_participant(self, request, pk=None, user_id=None):
        ...
        relation = CalendarRelation.objects.filter(
            calendar=calendar, content_type=ct, object_id=user_id
        ).first()
    ...
    def participants(self, request, pk=None):
        ...
        relations = CalendarRelation.objects.filter(
            calendar=calendar, content_type=ct
        )
    ...
    def export_ical(self, request, pk=None):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{calendar.slug}.ics"'
        )
    ...
    def import_ical(self, request, pk=None):
        ...
        if not user_can_edit_calendar(request.user, calendar):
            return Response(
                {
                    "error": (
                        "Только владелец может импортировать события "
                        "в календарь"
                    )
                },
                status=status.HTTP_403_FORBIDDEN,
                ...
                if component.name == "VEVENT":
                    try:
                        ...
                        if dtend:
                            ...
                            # Если нет времени окончания, делаем событие длиной
                            # 1 час
                        ...
                        errors.append(
                            f"Ошибка импорта события '{summary}': {str(e)}"
                        )
                        skipped_count += 1
...
class ScheduleEventViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if start_str and end_str:
            ...
            if start_dt and end_dt:
                ...
                qs = qs.filter(start__lte=end_dt, end__gte=start_dt)
    ...
    def occurrences(self, request):
        ...
        Query params:
                - calendar: ID календаря (необязательно,
                    если не указан - все доступные календари)
        ...
        for calendar in calendars:
            ...
            for occurrence in period.get_occurrences():
                # Создаем уникальный ID для каждого вхождения
                # (комбинация event_id и start)
                unique_id = (
                    f"{occurrence.event.id}_{occurrence.start.isoformat()}"
                )
    ...
    def my_events(self, request):
        ...
        Возвращает только события,
        где пользователь является участником (EventRelation).
        """
        from django.contrib.auth import get_user_model
