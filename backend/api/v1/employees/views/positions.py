...
class PositionViewSet(HistoryActionMixin, viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        """Права доступа: чтение для всех аутентифицированных,
        изменения для админов.
        """
    ...
    def set_groups(self, request, pk=None):
        ...
        POS-группа должности автоматически вкладывается
        в соответствующие AD-группы через сигнал m2m_changed
        для Position.groups (см. signals/ldap/position.py).
        """
        pos = self.get_object()
    ...
    def remove_groups(self, request, pk=None):
        ...
        через сигнал m2m_changed для Position.groups
        (см. signals/ldap/position.py).
        """
        pos = self.get_object()
        qs, err = self._validate_groups_payload(request)
        if err:
    ...
    def permissions(self, request, pk=None):
        """Возвращает все права, которые получает должность
        через свои группы.
        """
        pos = self.get_object()
        perms = (
