...
def _format_employee(obj: Any) -> Dict[str, Any]:
    ...
    return {
        "model_name": "employee",
        "object_id": obj.pk,
        "title": f"{obj.last_name} {obj.first_name} {
            obj.patronymic or ''
        }".strip(),
...
def _format_message(obj: Any) -> Dict[str, Any]:
    ...
    return {
        "model_name": "message",
        "object_id": obj.pk,
        "title": f"Сообщение от {obj.author}"
        if hasattr(obj, "author")
        else f"Сообщение #{obj.pk}",
...
def _format_equipment(obj: Any) -> Dict[str, Any]:
    ...
    return {
        ...
        "title": obj.name,
        "description": (
            f"Инв. №: {obj.inventory_number}, Сер. №: {obj.serial_number}"
