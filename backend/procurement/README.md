...
Модуль для управления внутренними закупками по отделам и учета оборудования/инвентаря.

## Основные возможности

### 1. Заявки на закупку
- Создание заявок с позициями (товары/услуги)
- Автоматический маршрут согласования по сумме:
  - До 10,000₽ — только руководитель отдела
  - 10,000-50,000₽ — руководитель отдела + финансовый менеджер
  - Свыше 50,000₽ — руководитель отдела + фин. менеджер + директор
- Отслеживание статуса: черновик → согласование → одобрено → в работе → завершено
...
- Статусы: доступно, в использовании, на обслуживании, в ремонте, списано, утеряно
- Привязка к отделам и ответственным лицам
- Отслеживание гарантийных сроков

### 4. Техническое обслуживание
- История обслуживания оборудования
- Типы ТО: осмотр, плановое ТО, ремонт, модернизация, чистка
...
- ✅ Созданы Celery задачи для асинхронной отправки email (tasks.py)
  - `send_approval_request_email` - запрос на согласование
  - `send_status_change_email` - изменение статуса заявки
  - `send_approval_notification_email` - уведомление о согласовании
  - `send_budget_alert_email` - предупреждение о бюджете
- ✅ Интегрированы уведомления в ViewSet actions
  - При submit: отправка email всем согласующим (delay())
  - При approve: уведомление создателя + следующего согласующего (delay())
  - При reject: уведомление создателя с причиной (delay())
...
1. **При создании заявки**: Уведомление руководителю отдела
2. **При отправке на согласование**: Email всем согласующим (Celery)
3. **При одобрении этапа**: Уведомление создателю + следующему согласующему
4. **При полном одобрении**: Email создателю о возможности приступить к закупке
5. **При отклонении**: Email создателю с комментариями
6. **При превышении бюджета**: Email руководителю и финансовому директору (задача готова)
...
- Создать email-шаблоны (HTML) для красивых писем
...
- **Async Tasks**: Celery + Redis (настроено)
