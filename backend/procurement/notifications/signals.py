...
notify.send() → channels.py → Celery → WebSocket/Email/Push
"""
...
def create_request_notifications(sender, instance, created, **kwargs):
    ...
    Обрабатывает:
    1. Новая адресная заявка - уведомление отделу-исполнителю
    ...
    try:
        ...
        if created:
            if (
                request.status == ProcurementStatus.WAITING
                and request.processing_department_id
                and not getattr(
                    request,
                    "_suppress_created_procurement_notifications",
                    False,
                )
            ):
                notify_processing_department_request(
                    request,
                    actor=getattr(request, "_notification_actor", None),
                )
            return
...
def _handle_status_change(request, new_status):
    ...
    if new_status == ProcurementStatus.PENDING:
        ...
        # Заявка отклонена
        if not getattr(request, "_requestor_rejected_notified", False):
...
def create_approval_notifications(sender, instance, created, **kwargs):
    ...
    Обрабатывает:
    1. Одобрение - уведомление создателю заявки и следующему этапу
    2. Отклонение - уведомление создателю заявки
    """
    try:
        approval = instance

        if created:
            # Нотификацию при submit запускает сигнал заявки, когда уже
            # созданы все Approval и можно определить только текущий этап.
            return
