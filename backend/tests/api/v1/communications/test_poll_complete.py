...
from .test_communications_api import (auth_client, department, department_chat,
                                      group_chat, private_chat, user1, user2,
                                      user3)
...
class TestPollCreationAPI:
    ...
    def test_create_anonymous_poll_via_api(
    ...
    def test_create_multiple_choice_poll_via_api(
        self, auth_client, private_chat, user1
    ):
        ...
        data = {
            ...
            'options_data': ['Option 1', 'Option 2', 'Option 3'],
    ...
    def test_create_quiz_via_api(self, auth_client, private_chat, user1):
        ...
        data = {
            ...
            'options_data': ['3', '4', '5'],
            'is_quiz': True,
            'correct_option_index': 1  # Правильный ответ: 4
        }
    ...
    def test_create_poll_with_auto_close_time(
        self, auth_client, private_chat, user1
    ):
        ...
        data = {
            ...
            'options_data': ['Yes', 'No'],
            'closes_in_minutes': 60  # Закрытие через 1 час
        }
    ...
    def test_create_poll_without_chat_id_fails(self, auth_client):
        """Попытка создать poll без chat_id должна провалиться"""
        url = '/api/v1/communications/polls/'
        data = {
            'question': 'Poll without chat',
            'options_data': ['Yes', 'No']
        }
    ...
    def test_create_poll_unauthenticated_fails(
        self, client, private_chat, user1
    ):
...
class TestPollVoting:
    ...
    def test_vote_single_choice(self, auth_client, simple_poll, user1):
        ...
        assert opt1.id in results['user_voted_option_ids']
    ...
    def test_revote_replaces_previous(self, auth_client, simple_poll):
        ...
        assert response.status_code == status.HTTP_200_OK
        results = response.data['results']
        assert opt2.id in results['user_voted_option_ids']
        assert opt1.id not in results['user_voted_option_ids']
...
class TestMultipleChoicePolls:
    ...
    def test_vote_multiple_options(self, auth_client, multiple_choice_poll):
        ...
        assert response.status_code == status.HTTP_200_OK
        results = response.data['results']
        assert len(results['user_voted_option_ids']) == 2
        assert opt1.id in results['user_voted_option_ids']
        assert opt2.id in results['user_voted_option_ids']
    ...
    def test_vote_all_options(self, auth_client, multiple_choice_poll):
        ...
        assert response.status_code == status.HTTP_200_OK
        results = response.data['results']
        assert len(results['user_voted_option_ids']) == 3
    ...
    def test_revote_multiple_choice(self, auth_client, multiple_choice_poll):
        ...
        response = auth_client.post(
            url, {'option_ids': [opt3.id]}, format='json'
        )

        assert response.status_code == status.HTTP_200_OK
        results = response.data['results']
        assert opt3.id in results['user_voted_option_ids']
        assert opt1.id not in results['user_voted_option_ids']
        assert opt2.id not in results['user_voted_option_ids']
...
class TestPollEdgeCases:
    ...
    def test_poll_without_options_can_be_created(
        self, auth_client, private_chat, user1
    ):
        """Можно создать голосование без опций (options_data пустой)"""
        url = '/api/v1/communications/polls/'
        data = {
            'chat_id': private_chat.id,
    ...
    def test_total_voters_count_unique_users(
        self, auth_client, private_chat, user1
    ):
