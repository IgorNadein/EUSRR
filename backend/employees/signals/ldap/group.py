...
def sync_group_to_ldap_on_save(sender, instance, created, **kwargs):
    ...
    try:
        ...
        if created:
            ...
            if not dn:
                logger.debug(f"Group {instance.id} has no LDAP DN, skipping sync")
                return
        ...
        logger.error(
            f"LDAP sync failed for Group {instance.id} (created={created}): {e}",
            exc_info=True
        )
    except Exception as e:
        logger.error(
...
def sync_group_members_to_ldap(sender, instance, action, pk_set, **kwargs):
    ...
    try:
        ...
        logger.error(
            f"Unexpected error in LDAP member sync for Group {instance.id}: {e}",
            exc_info=True
        )
