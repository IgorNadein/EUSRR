...
def sync_employee_to_ldap_on_save(sender, instance, created, **kwargs):
    ...
    Использует временные атрибуты _ldap_changes и _ldap_avatar
    для передачи данных.
    """
    ...
    if not sync_state or not (sync_state.ldap_dn or sync_state.ldap_guid):
        ...
        logger.debug(
            f"Employee {instance.id} has no LDAP sync state, skipping sync"
        )
        return
...
def sync_employee_to_ldap_on_delete(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.error(
            f"LDAP delete failed for Employee {instance.id}: {e}", exc_info=True
        )
        _enqueue(
            "employee_delete",
            "employee",
