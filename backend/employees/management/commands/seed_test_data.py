...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if clear_data:
            self.stdout.write(
                self.style.WARNING("Очистка существующих данных...")
            )
        ...
        if created:
            ...
            admin_user.save()
            self.stdout.write(
                self.style.WARNING(f"✓ Администратор {admin_email} обновлен")
            )
        ...
        for name in dept_names:
            dept, created = Department.objects.get_or_create(
                name=name, defaults={"description": f"Описание {name}"}
            )
        ...
        for (
            ...
            if user_created:
                ...
                EmployeeDepartment.objects.create(
                    employee=user, department=dept, is_active=True
                )
