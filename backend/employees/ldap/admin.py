...
class LdapOrganizationalUnitAdmin(admin.ModelAdmin):
    ...
    def dn_display(self, obj):
        ...
        for part in parts:
            if part.startswith('OU='):
                ...
                html_parts.append(format_html(
                    '<span style="color:#999;">{}</span>', part,
                ))
            else:
                html_parts.append(part)
        return format_html(','.join(html_parts))
    ...
    def managed_by_link(self, obj):
        ...
        if not mb:
            return format_html(
                '<em style="color:#999;">Не назначен</em>'
            )
        ...
        try:
            ...
            return format_html(
                ...
                url, ldap_user.display_name or ldap_user.cn,
                ldap_user.sam_account_name or '', mb,
            )
        except LdapUser.DoesNotExist:
            return format_html(
                '<code style="color:#666;">{}</code>', mb,
            )
    ...
    def django_department_link(self, obj):
        ...
        if dept:
            ...
            return format_html(
                '<a href="{}" style="color:#0066cc;">🏢 {}</a>', url, dept.name,
            )
        return format_html('<em style="color:#999;">—</em>')
    ...
    def django_department_info(self, obj):
        ...
        return format_html(
            ...
            dept.name,
            dept.head or '—',
            sync_state.updated_at.strftime('%Y-%m-%d %H:%M:%S') if sync_state else '—',
        )
    ...
    def sync_from_ldap_to_django(self, request, queryset):
        ...
        for ldap_ou in ldap_ous:
            ...
            try:
                ...
                error_count += 1
                self.message_user(
                    request, f'Ошибка синхронизации {ou_name}: {e}',
                    level=messages.ERROR,
                )
        ...
        if success_count:
            self.message_user(
                request,
                f'Успешно синхронизировано: {success_count} OU (LDAP → Django)',
                level=messages.SUCCESS,
            )
        if error_count:
            self.message_user(
    ...
    def sync_from_django_to_ldap(self, request, queryset):
        ...
        for ldap_ou in ldap_ous:
            ...
            try:
                ...
                error_count += 1
                self.message_user(
                    request, f'Ошибка синхронизации {ldap_ou.ou}: {e}',
                    level=messages.ERROR,
                )
        ...
        if success_count:
            self.message_user(
                request,
                f'Успешно синхронизировано: {success_count} OU (Django → LDAP)',
                level=messages.SUCCESS,
            )
        if error_count:
            self.message_user(
    ...
    def show_sync_diff(self, request, queryset):
        ...
        for ldap_ou in ldap_ous:
            dept = self._find_django_department(ldap_ou)
            if not dept:
                diffs.append(f'{ldap_ou.ou}: не найден связанный Django Department')
                continue
            ...
            if ldap_desc != (dept.description or ''):
                ou_diffs.append(
                    f"Описание: LDAP='{ldap_desc[:50]}' vs Django='{(dept.description or '')[:50]}'"
                )
            ...
            if dept.head:
                ...
                ou_diffs.append(f"Руководитель: LDAP='{mb}' vs Django='не назначен'")
    ...
    def delete_selected_ldap(self, request, queryset):
        ...
        if not ous:
            self.message_user(
                request, 'Не удалось найти выбранные OU.',
                level=messages.WARNING,
            )
            return
        ...
        for ou_obj in ous:
            try:
                ...
                self.message_user(
                    request, f'Ошибка удаления {ou_obj.ou}: {e}',
                    level=messages.ERROR,
                )
        if deleted:
            self.message_user(
                request, f'Удалено: {deleted} OU из LDAP.',
                level=messages.SUCCESS,
            )
        ...
        class FilteredChangeList(ChangeList):
            ...
            def get_results(self, request):
                ...
                if hasattr(self.model_admin, '_exclude_base_dn'):
                    ...
                    self.result_list = [
                        obj for obj in self.result_list
...
class LdapOrganizationalUnitGroupAdmin(admin.ModelAdmin):
    ...
    def members_count(self, obj):
        ...
        return format_html(
            '<strong style="color:#28a745;">{}</strong>', count,
        )
    ...
    def dn_display(self, obj):
        ...
        for part in parts:
            if part.startswith('CN='):
                html_parts.append(format_html(
                    '<strong style="color:#28a745;">{}</strong>', part,
                ))
            elif part.startswith('OU='):
                html_parts.append(format_html(
                    '<span style="color:#0066cc;">{}</span>', part,
                ))
            elif part.startswith('DC='):
                html_parts.append(format_html(
                    '<span style="color:#999;">{}</span>', part,
    ...
    def django_department_info(self, obj):
        ...
        url = reverse(
            'admin:employees_department_change', args=[dept.pk],
        )
        return format_html(
            ...
            '<td>{}</td></tr>'
            '</table></div>',
            dept.pk, url, dept.name, dept.head or '—',
        )
    django_department_info.short_description = 'Связь с Django'
...
class LdapGroupAdmin(admin.ModelAdmin):
    ...
    def group_type_display(self, obj):
        ...
        if 'OU=Groups' in dn:
            ...
            if dept:
                return format_html(
                    '<span style="color:#9c27b0;">🎭 Роль ({})</span>',
                    dept
                )
            return format_html(
                '<span style="color:#9c27b0;">🎭 Роль</span>'
            ...
            return format_html(
                '<span style="color:#999;">👥 Группа</span>'
            )
