...
def _conn() -> Connection:
    ...
    Returns:
        Connection: Готовое соединение.

    Raises:
        RuntimeError: Неверные настройки подключения.
    """
    ...
    if not uri or not bind_dn or not bind_pw:
        raise RuntimeError("LDAP_URI/LDAP_BIND_DN/LDAP_BIND_PASSWORD must be set")

    ca_file = getattr(settings, "LDAP_CA_CERTS", "") or None
    tls = Tls(
        validate=ssl.CERT_REQUIRED if ca_file else ssl.CERT_NONE, ca_certs_file=ca_file
    )
    server = Server(uri, use_ssl=uri.lower().startswith("ldaps://"), tls=tls)
    return Connection(
        server, user=bind_dn, password=bind_pw, auto_bind=True, receive_timeout=30
    )


@contextmanager
def _ldap() -> Connection:
    ...
    Notes:
        Любые ошибки закрытия глушатся — не маскируют основной результат.
    """
    conn = _conn()
    try:
        yield conn
