...
# ===== МОДЕЛИ ДЛЯ ИНТЕГРАЦИИ МЕССЕНДЖЕРОВ =====

class MessengerIntegration(models.Model):
    """
    Настройки интеграции с внешними мессенджерами (WhatsApp, Telegram, WeChat)
    """
    MESSENGER_TYPES = [
        ('whatsapp', 'WhatsApp'),
        ('telegram', 'Telegram'),
    ...
    # API настройки (зашифрованы в production)
    ...
    messages_sent = models.IntegerField(
        default=0,
        verbose_name="Отправлено сообщений"
    )
    
    messages_received = models.IntegerField(
        default=0,
        verbose_name="Получено сообщений"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['messenger_type', 'is_enabled']),
            models.Index(fields=['status']),
        ]
    
    def __str__(self):
        return f"{self.get_messenger_type_display()} - {self.get_status_display()}"


class MessengerAccount(models.Model):
    """
    Привязка аккаунта сотрудника к мессенджеру
    """
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
    ...
    external_id = models.CharField(
        max_length=200,
        verbose_name="ID в мессенджере"
    )
    
    # Имя/никнейм в мессенджере
    external_username = models.CharField(
        max_length=200,
        blank=True,
        verbose_name="Имя в мессенджере"
    )
    ...
    def __str__(self):
        return f"{self.user} - {self.integration.get_messenger_type_display()} ({self.external_username or self.external_id})"


class MessengerMessage(models.Model):
    """
    Синхронизированные сообщения из внешних мессенджеров
    """
    DIRECTION_CHOICES = [
        ('incoming', 'Входящее'),
        ('outgoing', 'Исходящее'),
    ...
    # Связь с внутренним сообщением (если синхронизировано)
    internal_message = models.OneToOneField(
        Message,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name='messenger_sync',
        verbose_name="Внутреннее сообщение"
    )
    
    # ID сообщения во внешнем мессенджере
    external_id = models.CharField(
        max_length=200,
        verbose_name="ID в мессенджере"
    )
    ...
    external_timestamp = models.DateTimeField(
        verbose_name="Время в мессенджере"
    )
    ...
    # Дополнительные данные из мессенджера
    ...
    class Meta:
        ...
        unique_together = [('integration', 'external_id')]
        indexes = [
            models.Index(fields=['integration', 'external_timestamp']),
            models.Index(fields=['sender_account', 'external_timestamp']),
            models.Index(fields=['status']),
    ...
    def __str__(self):
        return f"{self.integration.get_messenger_type_display()} - {self.external_id}"
