...
class ChatReadState(models.Model):
    ...
    last_read_message = models.ForeignKey(
        ...
        help_text=(
            "Обновляется автоматически при загрузке сообщений "
            "(Telegram-style)"
        ),
    )
    ...
    unread_count = models.IntegerField(
        ...
        help_text=(
            "Кешированное значение для производительности. "
            "Обновляется при новых сообщениях и прочтении."
        ),
    )

    # Статус набора текста
    is_typing = models.BooleanField(
        default=False, verbose_name="Набирает сообщение"
    )
...
class Chat(models.Model):
    ...
    type = models.CharField(
        ...
        db_index=True,
        help_text=(
            "Chat type: private, group, channel, announcement, "
            "global, comments. "
    ...
    # Позволяет привязать чат к ЛЮБОЙ модели (Project, Team, Event, Document,
    # etc.)
    ...
    flags = models.JSONField(
        ...
        verbose_name="Flags",
        help_text=(
            "Custom flags: {'is_primary': true, "
            "'is_archived': false, etc.}"
        ),
    )
    ...
    is_blocked = models.BooleanField(
        ...
        help_text=(
            "Для announcement: заблокирован администратором, "
            "скрыт у всех"
        ),
    )
    blocked_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Дата блокировки"
    )
    ...
    def mark_read(self, user):
        ...
        read_state, created = ChatReadState.objects.get_or_create(
            chat=self,
            user=user,
            defaults={"last_read_message": last_message, "unread_count": 0},
        )
    ...
    def unread_count_for(self, user):
        ...
        ОПТИМИЗИРОВАНО: Использует денормализованное поле
        unread_count из ChatReadState
    ...
    def get_participants(self):
        ...
        resolver_path = getattr(
            settings, "COMMUNICATIONS_PARTICIPANT_RESOLVER", None
        )
        ...
        if resolver_path:
            try:
                ...
                logger.warning(
                    "Failed to import participant resolver "
                    f"'{resolver_path}': {e}"
                )
    ...
    def __str__(self):
        if self.type == "private":
            ...
            names = (
                ", ".join(
                    f"{m.user.last_name} {m.user.first_name}".strip()
                    for m in members
                )
                or "—"
            )
...
class ChatUserSettings(models.Model):
    ...
    chat = models.ForeignKey(
        Chat, on_delete=models.CASCADE, related_name="user_settings"
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
    ...
    pinned_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Время закрепления"
    )
    ...
    custom_name = models.CharField(
        max_length=255, blank=True, verbose_name="Свое название"
    )
...
class Message(models.Model):
    ...
    is_cross_chat = models.BooleanField(
        default=False, help_text="Сообщение отправлено не участником чата"
    )
...
class MessageAttachment(models.Model):
    ...
    file = models.FileField(
        upload_to="chat_attachments/%Y/%m/%d/", verbose_name="Файл"
    )
    file_type = models.CharField(
        max_length=10, choices=FILE_TYPE_CHOICES, verbose_name="Тип файла"
    )
    ...
    mime_type = models.CharField(
        max_length=100, blank=True, verbose_name="MIME тип"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            ...
            and self.file
            and (not self.width or not self.height)
        ):
            try:
                ...
                # Открываем файл
                file_obj = (
                    self.file.file if hasattr(self.file, "file") else self.file
                )
...
class MessageEditHistory(models.Model):
    ...
    edited_at = models.DateTimeField(
        auto_now_add=True, verbose_name="Время редактирования", db_index=True
    )
...
class MessageForwardMetadata(models.Model):
    ...
    forwarded_at = models.DateTimeField(
        auto_now_add=True, verbose_name="Время пересылки"
    )
    ...
    original_created_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Дата создания оригинала"
    )
    original_chat_name = models.CharField(
        max_length=255, blank=True, verbose_name="Название оригинального чата"
    )
...
class ChatMembership(models.Model):
    ...
    can_send_messages = models.BooleanField(
        default=True, verbose_name="Может отправлять сообщения"
    )
    can_add_members = models.BooleanField(
        default=False, verbose_name="Может добавлять участников"
    )
    can_remove_members = models.BooleanField(
        default=False, verbose_name="Может удалять участников"
    )
    can_pin_messages = models.BooleanField(
        default=False, verbose_name="Может закреплять сообщения"
    )
...
class Poll(models.Model):
    ...
    is_quiz = models.BooleanField(
        ...
        help_text=(
            ...
            "показывается после голосования"
        ),
    )
    ...
    is_closed = models.BooleanField(
        default=False, verbose_name="Голосование закрыто"
    )
...
class PollOption(models.Model):
    ...
    is_correct = models.BooleanField(
        default=False, verbose_name="Правильный ответ", help_text="Для викторин"
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Создан")
