    ...
    def handle(self, *args, **options):
        ...
        if has_department_field:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        "   ⚠ Модуль 'employees' не установлен, "
                        "пропускаем проверку department"
                    )
                )
                dept_ct = None
            ...
            if dept_ct:
                ...
                self.stdout.write(
                    f"   Найдено чатов с department: {count_dept}"
                )
                ...
                for chat in chats_with_dept:
                    ...
                    if not chat.context_object_id:
                        errors.append(
                            f"Chat #{
                                chat.id
                            }: есть department но context_object_id is NULL"
                        )
                    elif chat.context_object_id != chat.department_id:
                        errors.append(
                            f"Chat #{chat.id}: context_object_id ({
                                chat.context_object_id
                            }) "
                        ...
                        errors.append(
                            f"Chat #{
                                chat.id
                            }: context_content_type не Department"
                        )
                    elif verbose:
                        self.stdout.write(
                            f"   ✓ Chat #{chat.id}: department={
                                chat.department_id
                            } "
                ...
                if not errors and count_dept > 0:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"   ✓ Все {count_dept} чатов с department "
                            "мигрированы корректно"
                        )
                    )
                elif count_dept == 0:
                    self.stdout.write(
                        self.style.WARNING("   ⚠ Чатов с department не найдено")
                    )
        else:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "   ✓ Поле 'department' удалено, "
                    f"чатов с context: {chats_with_context}"
                )
            )
        ...
        if has_is_main_field:
            ...
            self.stdout.write(
                f"   Найдено главных чатов (is_main=True): {count_main}"
            )

            for chat in main_chats:
                flags_primary = (
                    chat.flags.get("is_primary") if chat.flags else False
                )
                ...
                if not flags_primary:
                    errors.append(
                        f"Chat #{
                            chat.id
                        }: is_main=True но flags['is_primary'] не установлен"
                    )
                elif verbose:
                    self.stdout.write(
                        f"   ✓ Chat #{
                            chat.id
                        }: is_main=True → flags['is_primary']=True"
                    )

            if not errors and count_main > 0:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"   ✓ Все {count_main} главных чатов "
                        "мигрированы в flags корректно"
                    )
                )
            elif count_main == 0:
                self.stdout.write(
                    self.style.WARNING("   ⚠ Главных чатов не найдено")
                )
        else:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "   ✓ Поле 'is_main' удалено, "
                    f"чатов с flags.is_primary: {chats_with_primary}"
                )
            )

        # ===== 3. Проверка include_all_users =====
        self.stdout.write(
            "\n" + self.style.HTTP_INFO("3. Проверка поля include_all_users...")
        )
        ...
        if has_old_field and not has_new_field:
            ...
            self.stdout.write(
                self.style.WARNING(
                    "   ⚠ Старое поле 'include_all_employees' существует, "
                    f"чатов: {count}"
                )
            )
        elif has_new_field:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "   ✓ Поле 'include_all_users' существует, "
                    f"чатов использующих: {count}"
                )
            )
        else:
            errors.append(
                "Ни 'include_all_employees' ни 'include_all_users' не найдены!"
            )
        ...
        with_flags_primary = Chat.objects.filter(flags__is_primary=True).count()
