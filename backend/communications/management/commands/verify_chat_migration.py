            ...
            if dept_ct:
                ...
                for chat in chats_with_dept:
                    ...
                    if not chat.context_object_id:
                        errors.append(
                            f"Chat #{
                                chat.id}: есть department но context_object_id is NULL")
                    elif chat.context_object_id != chat.department_id:
                        errors.append(
                            f"Chat #{
                                chat.id}: context_object_id ({
                                chat.context_object_id}) " f"!= department_id ({
                                chat.department_id})")
        ...
        if has_is_main_field:
            ...
            for chat in main_chats:
                ...
                if not flags_primary:
                    errors.append(
                        f"Chat #{
                            chat.id}: is_main=True но flags['is_primary'] не установлен")
                elif verbose:
                    self.stdout.write(
