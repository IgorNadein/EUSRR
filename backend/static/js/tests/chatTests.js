...
class ChatTestSuite {
    ...
    async testMessageStore() {
        ...
        store.on('message_added', listener);
        ...
        store.off('message_added', listener);
    ...
    async testChatController() {
        ...
        controller.store.on('message_added', handler);
    ...
    // ==================== Тесты умного автоскролла ====================

    async testSmartAutoscroll() {
        console.log('\n🧪 Тестирование умного автоскролла и индикатора новых сообщений...\n');
        this.resetResults();
        ...
        try {
            ...
            console.log('[Test] Init failed (expected without backend):', e.message);
            // ВАЖНО: Сбрасываем флаг инициализации для тестов
            controller.isInitializing = false;
        }
        ...
        this.assert(controller.newMessagesCount > 0, 'Счётчик новых сообщений должен увеличиться');
        ...
        if (indicator) {
            ...
            console.warn('[Test] Индикатор не найден (может создаваться при первом показе)');
        }

        // Тест 4: Клик по индикатору - скролл вниз и скрытие
        console.log('\n--- Тест 4: Клик по индикатору ---');
        
        if (indicator && controller.newMessagesCount > 0) {
            ...
            this.assertEqual(controller.newMessagesCount, 0, 'Счётчик должен быть сброшен');
        ...
        // Прокручиваем вверх
        container.scrollTop = 50;
        controller.newMessagesCount = 5; // Устанавливаем счётчик вручную
        ...
        // Прокручиваем вниз вручную
        controller.scrollManager.scrollToBottom({ instant: false, force: true });
        
        // Увеличиваем ожидание для гарантированного срабатывания scroll watcher
        await this.sleep(500);
        
        // Проверяем что индикатор скрылся
        this.assertEqual(controller.newMessagesCount, 0, 'Счётчик должен сброситься при ручном скролле');
        ...
        this.assertEqual(controller.newMessagesCount, 3, 'Счётчик должен показывать 3 новых сообщения');
    ...
    async runAll() {
        ...
        try {
            ...
            // Тесты умного автоскролла
            const smartAutoscrollResults = await this.testSmartAutoscroll();

            // Суммируем результаты
            [storeResults, loaderResults, scrollManagerResults, rendererResults, 
             controllerResults, integrationResults, perfResults, advancedResults, 
             scrollStabilityResults, smartAutoscrollResults].forEach(result => {
...
window.ChatTests = {
    ...
    testSmartAutoscroll: () => chatTests.testSmartAutoscroll()
};
...
console.log('  window.ChatTests.testSmartAutoscroll()  - Тесты умного автоскролла');
