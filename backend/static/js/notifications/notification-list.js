...
class NotificationListManager {
    ...
    renderNotifications(data) {
        ...
        const notificationsHtml = data.notifications.map(n => this.renderNotificationCard(n)).join('');
    ...
    renderNotificationCard(notification) {
        ...
        return `
            <div class="col">
                <div class="card ${readClass}" data-notification-id="${notification.id}">
                    <div class="card-body">
                        <div class="d-flex align-items-start">
                            ...
                            <div class="flex-grow-1 ms-3">
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    ...
                                    <small class="text-muted ms-2">${this.formatTime(notification.created_at)}</small>
                                </div>
                                ...
                                <div class="d-flex gap-2">
                                    ${notification.action_url ? `
                                        <a href="${notification.action_url}" class="btn btn-sm btn-outline-primary">
                                            <i class="bi-arrow-right-circle"></i> ${notification.action_text || 'Посмотреть'}
                                        </a>
                                    ` : ''}
    ...
    async deleteNotification(id) {
        try {
            ...
            if (response.ok) {
                // Удалить карточку
                const card = document.querySelector(`[data-notification-id="${id}"]`)?.closest('.col');
                if (card) {
                    card.remove();
                }
