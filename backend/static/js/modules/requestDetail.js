...
export class RequestDetailModal {
  constructor(
    modalId = "requestDetailModal",
    containerId = "requestDetailContent"
  ) {
  ...
  async open(requestId) {
    ...
    if (this.container) {
      this.container.innerHTML =
        '<div class="d-flex justify-content-center p-4"><div class="spinner-border" role="status"><span class="visually-hidden">Загрузка...</span></div></div>';
    }
  ...
  attachEventListeners() {
    ...
    const deleteCommentForms = this.container?.querySelectorAll(
      '[action*="request_comment_delete"]'
    );
  ...
  async handleCommentSubmit(e) {
    ...
    try {
      ...
      if (submitBtn) {
        ...
        submitBtn.innerHTML =
          '<i class="bi-check-circle me-2"></i>Комментарий добавлен!';
  ...
  updateCommentsCount() {
    ...
    if (commentsContainer && countSpan) {
      const commentCount = commentsContainer.querySelectorAll(
  ...
  async handleStatusChange(e) {
    ...
    const confirmMsg =
      ...
      `Вы уверены, что хотите изменить статус на "${
        form.querySelector(`option[value="${newStatus}"]`)?.textContent
      }"?`;
    ...
    if (submitBtn) {
      submitBtn.disabled = true;
      submitBtn.innerHTML =
        '<span class="spinner-border spinner-border-sm me-2"></span>Обновление...';
    }
    ...
    try {
      ...
      if (submitBtn) {
        ...
        submitBtn.innerHTML =
          '<i class="bi-check-circle me-2"></i>Статус обновлен!';
