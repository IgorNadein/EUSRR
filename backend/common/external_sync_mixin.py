...
class ExternalSystemSyncMixin:
    ...
    def handle_external_sync_error(self, error, instance, action):
        ...
        """
        logger.error(
            f"External sync failed for {action} on {instance}: {error}")
        return Response(
    ...
    def _perform_with_sync(self, serializer, action, db_operation):
        ...
        with transaction.atomic():
            ...
            try:
                ...
                sync_method = getattr(
                    self.external_sync_service, sync_method_name)
                ...
                if isinstance(result, dict) and result.get('update_db'):
                    ...
                    instance.save(update_fields=list(
                        result['update_db'].keys()))
                
                # Вызываем кастомный обработчик результата если определен
                if hasattr(self, 'handle_external_sync_result'):
                    self.handle_external_sync_result(instance, result, action)

                logger.info(
                    f"External sync success for {action} on {instance}")
                ...
                error_response = self.handle_external_sync_error(
                    e, instance, action)
                if error_response:
