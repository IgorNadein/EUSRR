...
def create_request_notifications(sender, instance, created, **kwargs):
    ...
    print(f"🔔 [SIGNAL] create_request_notifications: created={created}, status={request_obj.status}, id={request_obj.id}")
    ...
    if created and request_obj.status != "draft":
        ...
        print(f"📌 [SIGNAL] Помечаем заявление #{request_obj.id} для отправки уведомлений после установки recipients")
...
def notify_on_recipients_changed(sender, instance, action, **kwargs):
    ...
    if action == "post_add" and instance.id in _pending_new_requests:
        print(f"🎯 [M2M_SIGNAL] Recipients установлены для заявления #{instance.id}, отправляем уведомления")
...
def notify_on_cc_users_changed(sender, instance, action, **kwargs):
    ...
    if action == "post_add" and instance.id in _pending_new_requests:
        ...
        if instance.recipients.count() == 0:
            print(f"🎯 [M2M_SIGNAL] CC users установлены для заявления #{instance.id} (без recipients), отправляем уведомления")
            _pending_new_requests.discard(instance.id)
            notify_new_request(instance)
...
def notify_new_request(request_obj):
    ...
    for recipient in request_obj.recipients.filter(is_active=True):
        ...
        print(f"   ✅ Добавлен основной получатель: {recipient.username} (ID: {recipient.id})")
    ...
    print(f"📊 [notify_new_request] Всего получателей для заявления #{request_obj.id}: {len(recipients_set)}")
    if len(recipients_set) == 0:
        print(f"⚠️  [notify_new_request] НЕТ ПОЛУЧАТЕЛЕЙ! Уведомления не будут отправлены.")
        return
