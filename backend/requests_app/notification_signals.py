...
def notify_new_request(request_obj):
    ...
    logger.info(f"[notify_new_request] Основные получатели (recipients): {recipients_count}")
    ...
    for recipient in request_obj.recipients.filter(is_active=True):
        ...
        logger.info(f"[notify_new_request] ✅ Основной получатель: {recipient.username} (ID={recipient.id}, email={recipient.email})")
        print(
    ...
    for cc_user in request_obj.cc_users.filter(is_active=True):
        ...
        logger.info(f"[notify_new_request] ✅ CC получатель: {cc_user.username} (ID={cc_user.id}, email={cc_user.email})")
    ...
    for r in recipients_set:
        logger.info(f"[notify_new_request]   👤 {r.username} (ID={r.id}, email={r.email})")
