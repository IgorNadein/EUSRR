...
def _fmt_date(val, fmt: str = "%d.%m.%Y") -> str | None:
    """Форматирует дату из строки 'YYYY-MM-DD' в читаемый вид"""
    if not val:
        return None
    try:
        from datetime import datetime
        if isinstance(val, str) and len(val) == 10:  # YYYY-MM-DD
...
# Словарь типов заявлений для отображения
...
def _add_type_display(data: Dict[str, Any]) -> Dict[str, Any]:
    """Добавляет get_type_display к данным заявления"""
    if isinstance(data, dict) and "type" in data:
        data["get_type_display"] = REQUEST_TYPE_DISPLAY.get(
            data["type"], data["type"]
        )
    return data
...
class RequestDetailView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, pk: int, **kwargs: Any) -> Dict[str, Any]:
        ...
        # Форматируем даты в request_data
        formatted_request = _format_datetime_fields(
            [request_data], fields=("created_at", "updated_at", "decided_at")
        )[0]
        
        # Форматируем date_from и date_to отдельно
        if formatted_request.get('date_from'):
            formatted_request['date_from'] = _fmt_date(formatted_request['date_from'])
        if formatted_request.get('date_to'):
            formatted_request['date_to'] = _fmt_date(formatted_request['date_to'])

        # Добавляем человекочитаемый тип
        _add_type_display(formatted_request)

        ctx.update(
            {
                "request_obj": formatted_request,
                "comments": comments,
                "can_process": _user_can_process_requests(request.user),
            }
        )
