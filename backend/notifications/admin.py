...
class NotificationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Временные метки", {"fields": ("timestamp",)}),
    )
    ...
    def mark_as_read(self, request, queryset):
        ...
        self.message_user(
            request, f"Отмечено как прочитанные: {count} уведомлений"
        )
    ...
    def mark_as_unread(self, request, queryset):
        ...
        self.message_user(
            request, f"Отмечено как непрочитанные: {count} уведомлений"
        )
    ...
    def mark_as_deleted(self, request, queryset):
        ...
        self.message_user(
            request, f"Помечено как удаленные: {count} уведомлений"
        )
...
class UserChannelPreferencesAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Email настройки", {"fields": ("email_frequency",)}),
        (
            'Режим "Не беспокоить"',
            {
                "fields": (
