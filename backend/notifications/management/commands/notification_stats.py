...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("\n📁 По категориям:")
        category_stats = (
            Notification.objects.values("notification_type__category__name")
            .annotate(
                count=Count("id"), unread=Count("id", filter=Q(is_read=False))
            )
            .order_by("-count")
        )
        ...
        for stat in category_stats[:10]:
            category_name = (
                stat["notification_type__category__name"] or "Unknown"
            )
            self.stdout.write(
        ...
        type_stats = (
            Notification.objects.values("notification_type__name")
        ...
        for stat in user_stats:
            ...
            self.stdout.write(
                f"  {email}: {stat['count']} (непрочитанных: {stat['unread']})"
            )
