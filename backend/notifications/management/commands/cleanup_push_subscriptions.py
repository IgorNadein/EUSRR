    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.WARNING(
                ...
                f"Очистка подписок старше {days} дней "
        ...
        high_error = WebPushSubscription.objects.filter(
            error_count__gte=10, updated_at__lt=cutoff_date
        )
        ...
        self.stdout.write(
            f"  С 10+ ошибками старше {days} дней: {high_error_count}"
        )
        ...
        if not dry_run:
            ...
            if high_error_count > 0:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        "✅ Удалено "
                        f"{deleted_errors} подписок с множественными ошибками"
                    )
                )
            ...
            self.stdout.write(
                self.style.WARNING(
                    ...
                    "Было бы удалено: "
                    f"{inactive_count + high_error_count} подписок"
                )
            )
        ...
        if active_count > 0:
            ...
            for device, count in sorted(
                device_stats.items(), key=lambda x: x[1], reverse=True
            ):
