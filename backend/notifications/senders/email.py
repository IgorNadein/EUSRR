...
class EmailNotificationSender(BaseNotificationSender):
    ...
    def can_send(self, notification, user_preferences) -> bool:
        ...
        if user_preferences.email_frequency != 'instant':
            self.log_skip(notification, f"email_frequency={user_preferences.email_frequency}")
            return False
    ...
    def send(self, notification, **kwargs) -> bool:
        ...
        try:
            ...
            html_message = self._render_template('notifications/email/notification.html', context)
            text_message = self._render_template('notifications/email/notification.txt', context)
    ...
    def send_digest(self, user, notifications: list, frequency: str = 'daily') -> bool:
        ...
        try:
            ...
            # Рендерим шаблоны
            html_message = self._render_template('notifications/email/digest.html', context)
            text_message = self._render_template('notifications/email/digest.txt', context)
