...
def get_event_recipients(event) -> list:
    ...
    relations = CalendarRelation.objects.filter(
        calendar=event.calendar, content_type=user_ct, object_id__isnull=False
    )

    # Собираем активных пользователей
    user_ids = relations.values_list("object_id", flat=True)
    recipients = list(Employee.objects.filter(id__in=user_ids, is_active=True))
...
def notify_event_created(event, creator=None):
    ...
    try:
        ...
        description = MessageTemplates.event_created(
            event.title, event_date, calendar_name
        )
...
def notify_event_changed(
    event, changed_fields: list[str] = None, modifier=None
):
    """
    ...
    try:
        ...
        changes_text = (
            format_changes(changed_fields) if changed_fields else None
        )
        ...
        logger.info(
            f"Отправлены уведомления об изменении события '{event.title}' для {
                len(recipients)
            } получателей"
        )
...
def notify_event_cancelled(event, canceller=None):
    ...
    try:
        ...
        logger.info(
            f"Отправлены уведомления об отмене события '{event.title}' для {
                len(recipients)
            } получателей"
        )
